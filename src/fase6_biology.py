...
try:
    ...
    if df.empty:
        ...
        exit(0)
    # Prendiamo il miglior cocktail generato (Rank 1)
    top_cocktail_str = df.iloc[0]["molecules"]
except Exception as e:
    print(f"Errore nel leggere il file results/validated_combinations.csv: {e}")
    exit(1)

molecules = ast.literal_eval(top_cocktail_str)
print(f"Analisi del Rank 1 Cocktail ({len(molecules)} principi attivi):")
print(f"-> {molecules}\n")
